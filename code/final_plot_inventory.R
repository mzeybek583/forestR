...
write.table(inv2, paste(f.name,"-inventory_treeLS.csv", sep = ""), sep = ";", row.names = FALSE, quote = FALSE)

mescere <- data.frame("ortalama yukseklik"=mean(inv2$H), "Ortalama cap"=mean(2*inv2$Radius), "Toplam Hacim"=sum(inv2$volume))
